 ...
 *
 * 摘除是逐格进行的，且**保证树里至少留下一片叶子**：`removeLeaf` 摘掉最后一片时返回 null，`?? root`
 * 把那一步整个撤销。所以「当前 Topic 一张 Tab 也没开」时不会得到空树——「没有布局」不是这一层能表达
 * 的状态，交给上层按「Topic 里没东西」处理。注意留下的是**恰好一片**、且是循环走到最后时还剩的那片，
 * 不是原树：两格都投影到空时结果是单叶，不是原来的两叶。曾经这段注释写的是「保留原树」，实测（两格
 * 都属于别的 Topic）拿到的是单叶——那个说法从来不成立，只是没人断言过所以一直读起来像已决之事。
 */
