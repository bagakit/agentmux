...
import { fanOutSlug } from '../../../shared/fanout-naming'
...
export function buildFanOutRequest(input: {
  ...
  // The stem identifies the group afterwards, so it comes from what the user asked for rather than a
  // timestamp or counter — a fan-out named `run-3` tells nobody what it was trying to do. Slugged through
  // the ONE naming function the authority uses too, so the two sides cannot disagree about the name.
  const baseName = fanOutSlug(prompt)
