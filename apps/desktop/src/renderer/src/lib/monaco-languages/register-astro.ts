...
export const ASTRO_LANGUAGE_ID = 'astro'
...
export function registerAstroLanguage(monaco: MonacoModule): void {
  const astroAlreadyRegistered = monaco.languages
    .getLanguages()
    .some((language) => language.id === ASTRO_LANGUAGE_ID)
  if (astroAlreadyRegistered) {
    return
  }

  monaco.languages.register({
    id: ASTRO_LANGUAGE_ID,
    extensions: ['.astro'],
    aliases: ['Astro']
  })
  monaco.languages.setMonarchTokensProvider(ASTRO_LANGUAGE_ID, astroMonarchLanguage)
  monaco.languages.setLanguageConfiguration(ASTRO_LANGUAGE_ID, astroLanguageConfiguration)
}
