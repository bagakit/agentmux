...
export function cssColorToOscRgb(value: string | undefined): string | null {
  ...
  const rgbBody = /^rgba?\(\s*([^)]+)\)$/i.exec(trimmed)?.[1]
  if (!rgbBody) return null
  const channels = parseCssRgbChannels(rgbBody)
...
function parseCssRgbChannel(component: string): number | null {
  ...
  if (percent !== undefined) return clampByte((Number(percent) / 100) * 255)
...
export function terminalOscColorQueryReply(
  ...
  const color = slot === 10
    ? cssColorToOscRgb(colors.foreground)
    : cssColorToOscRgb(colors.background)
...
function parseTerminalOscTerminator(
  ...
  if (offset >= data.length) return { kind: 'partial' }
  if (data[offset] === BEL) return { kind: 'complete', endIndex: offset + BEL.length }
  if (data.startsWith(STRING_TERMINATOR, offset)) {
    return { kind: 'complete', endIndex: offset + STRING_TERMINATOR.length }
  }
  if (data[offset] === '\x1b' && offset + 1 >= data.length) return { kind: 'partial' }
  return { kind: 'none' }
}
...
function completeTerminalOscColorQuery(
  ...
  if (terminator.kind !== 'complete') return terminator
...
function parseTerminalOscColorQueryBody(
  ...
  if (slot !== 10 || data[bodyStart + 1] !== ';') return { kind: 'none' }
  if (bodyStart + 2 >= data.length) return { kind: 'partial' }
  if (data[bodyStart + 2] !== '?') return { kind: 'none' }
...
export function parseTerminalOscColorQuery(
  ...
  return parseTerminalOscColorQueryBody(data, offset + entry.prefix.length, entry.slot)
}

export type TerminalOscColorQueryScan = {
  remainder: string
  replies: string[]
}

export function scanTerminalOscColorQueries(
  chunk: string,
  remainder: string,
  colors: TerminalOscColorQueryReplyColors
): TerminalOscColorQueryScan {
  ...
  while (offset < data.length) {
    ...
    if (oscIndex === -1) {
      return {
        replies,
        remainder: data.endsWith('\x1b') ? '\x1b' : ''
      }
    }
    const query = parseTerminalOscColorQuery(data, oscIndex)
    if (query.kind === 'partial') return { replies, remainder: data.slice(oscIndex) }
    if (query.kind === 'match') {
      for (const slot of query.slots) {
        const reply = terminalOscColorQueryReply(colors, slot)
