...
// ---------------------------------------------------------------------------
// 接线守卫：groupIds / regionIds 现在只是 SSOT 的**一句话转发**。折叠后的风险不是内容错（内容已被上面
// 那族行为断言钉住），而是「这层转发还在不在执行、还是不是纯转发」——本仓 extracting-to-lib-only-fixes-half：
// 把逻辑抽进 lib 让内容可测了，但那层转发有没有被执行、是不是只转发，照旧无人守。所以这里用 TS parser 问
