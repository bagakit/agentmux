...
describe('context observation to composer', () => {
  ...
  it('reflects reduced context after compaction and clamps exhausted remaining to zero', () => {
    expect(renderToStaticMarkup(<AgentContextUsage usage={observe(800)} />)).toContain('Context 20% left')
    expect(renderToStaticMarkup(<AgentContextUsage usage={observe(200)} />)).toContain('Context 80% left')
    expect(renderToStaticMarkup(<AgentContextUsage usage={observe(1100)} />)).toContain('Context 0% left')
  })
  it.each([undefined, 0, -1, '1000'])('keeps missing or invalid capacity %s unknown', (capacity) => {
    ...
    expect(renderToStaticMarkup(<AgentContextUsage usage={usage} />)).toContain('Context unknown')
  })
})
