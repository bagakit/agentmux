...
it('keeps PMO teams on one bottom entry and one canonical Topic', () => {
  ...
  expect(styles).toContain('pmo-teams-topic-firework-left')
  expect(styles).toContain('pmo-teams-topic-firework-right')
  expect(styles).toContain('pmo-teams-topic-compact-launcher__firework i:nth-child(6)')
  ...
  expect(panel).toContain('tabId: floating.targetTabId')
...
it('keeps PMO dragging in a frame-synchronous visual preview until pointer end', () => {
  const panel = readFileSync(join(renderer, 'components/PmoTeamsTopicFloatingPanel.tsx'), 'utf8')
  const moveStart = panel.indexOf('const onPanelPointerMove')
  const moveEnd = panel.indexOf('const onPanelPointerEnd', moveStart)
  const frameStart = panel.indexOf('const renderDragFrame')
  const frameEnd = panel.indexOf('const scheduleDragFrame', frameStart)
  ...
  const move = panel.slice(moveStart, moveEnd)
  const frame = panel.slice(frameStart, frameEnd)
  expect(move).toContain('scheduleDragFrame()')
  expect(move).not.toContain('setFloating(')
  expect(frame).toContain('panelRef.current.style.transform')
  expect(panel).toContain('window.requestAnimationFrame(renderDragFrame)')
  expect(panel).toContain('setFloating({ position: next.position })')
})
