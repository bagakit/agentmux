...
it('keeps PMO teams topic on one fixed floating/compact launcher and one canonical Topic', () => {
  ...
  const attachedTitlebarStart = styles.indexOf('.pmo-teams-topic-floating__titlebar--attached {')
  const attachedTitlebarEnd = styles.indexOf('.pmo-teams-topic-floating__titlebar:active', attachedTitlebarStart)
  expect(attachedTitlebarStart).toBeGreaterThan(-1)
  expect(attachedTitlebarEnd).toBeGreaterThan(attachedTitlebarStart)
  expect(styles.slice(attachedTitlebarStart, attachedTitlebarEnd)).toContain('min-height: 38px')
  expect(styles.slice(attachedTitlebarStart, attachedTitlebarEnd)).toContain('padding-left: 58px')
