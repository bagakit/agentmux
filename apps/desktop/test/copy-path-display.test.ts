 ...
 *      **不是** `startsWith(home)`，否则 `/tmp/alice-homeOTHER/x` 会被剥成 `~OTHER/x`（点名的反例）。
...
describe('abbreviateHomePath: 缩写只按真实 home 值判边界', () => {
  const HOME = '/tmp/alice-home'

  it('路径恰好是 home 本身 → ~', () => {
    expect(abbreviateHomePath('/tmp/alice-home', HOME)).toBe('~')
  })

  it('home + "/" 前缀 → ~ 接其余部分（用户报的那种）', () => {
    expect(abbreviateHomePath('/tmp/alice-home/proj/github/pi', HOME)).toBe('~/proj/github/pi')
  })

  it('别人的家目录不被误缩写——判据是 home 值不是路径长相（设计 SSOT 点名的反例）', () => {
    // 变异靶子 (a)：把边界写成 `startsWith(home)`，这一条立刻红——它会切成 `~OTHER/x`。
    expect(abbreviateHomePath('/tmp/alice-homeOTHER/x', HOME)).toBe('/tmp/alice-homeOTHER/x')
  })

  it('home 是前缀但下一段不是路径分隔 → 原样（同一反例的另一面）', () => {
    // `/tmp/alice-home-backup` 以 `home` 开头，但 home 后面不是 `/`，所以不是 home 之内。
    expect(abbreviateHomePath('/tmp/alice-home-backup/x', HOME)).toBe('/tmp/alice-home-backup/x')
  })
  ...
  it('home 为空（本机 home 未知）→ 一律原样，绝不拿空串去猜', () => {
    expect(abbreviateHomePath('/tmp/alice-home/proj', '')).toBe('/tmp/alice-home/proj')
  })
})
...
describe('applyCopyPathStyle: 开关与逐行缩写', () => {
  ...
  it('默认档（copyPathsAsAbsolute 缺席）缩写', () => {
    expect(applyCopyPathStyle('/tmp/alice-home/proj/pi', { home: HOME, copyPathsAsAbsolute: undefined }))
      .toBe('~/proj/pi')
  })

  it('默认档（copyPathsAsAbsolute === false）也缩写', () => {
    expect(applyCopyPathStyle('/tmp/alice-home/proj/pi', { home: HOME, copyPathsAsAbsolute: false }))
      .toBe('~/proj/pi')
  })

  it('开关打开（copyPathsAsAbsolute === true）→ 原样绝对路径，一个字符都不动', () => {
    // 变异靶子 (b)：让实现忽略开关（永远缩写），这一条立刻红。
    expect(applyCopyPathStyle('/tmp/alice-home/proj/pi', { home: HOME, copyPathsAsAbsolute: true }))
      .toBe('/tmp/alice-home/proj/pi')
  })

  it('多选逐行各自缩写，用换行拼接', () => {
    const input = '/tmp/alice-home/a.ts\n/tmp/alice-homeOTHER/b.ts\n/etc/c'
