...
it('keeps executor avatar customization across a fresh ConfigStore instance', async () => {
  ...
  const avatars = { codex: { tint: '#ee7755', badge: 'A' }, review: { tint: '#6688dd', badge: 'B' } }
...
it.each([{ tint: 'red; display:none' }, { badge: 'too long' }])('rejects malformed avatar customization %j without overwriting durable settings', async (avatar) => {
  const root = await mkdtemp(join(tmpdir(), 'avatar-config-')); roots.push(root)
  const path = join(root, 'config.json')
  const store = new ConfigStore(path)
