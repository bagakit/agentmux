...
//
//   1. 行为层：那份判定本身对不对（哪些矩形被拒、被接受的怎么归一）。**在本文件里。**
//   2. 接线层（行为）：两个调用点真的执行到它，且交出去的就是它的答案。**不在本文件里**——它需要各自
//      那一侧的替身（renderer 要 apply spy，main 要假 WebContentsView），而那两套替身在各自的 suite
//      里已经有了，搬过来等于第三份。落点是 browser-bounds-sync.test.ts 的「接线层：壳交出去的就是共用
//      判定的答案」与 browser-view-manager.test.ts 的 `forwards the shared decision byte-for-byte`。
//      本文件此前的头声称这一层也在这儿，那是假的（#713）：整个文件对两个壳只有下面的静态读源码，
//      一条都不驱动它们。
...
//     结构层看不见。第 2 层的恒等断言仍能钉住取值正确，代价是重复回来了而无人报警。
//   - 结构层的「没有 `Math.` 运算」是按 `Math.xxx()` 这个形状判的。`const M = Math; M.round(...)` 或
//     `import { round } from …` 能绕过。这不是"禁止清单"式的穷举（那必漏，见
//     forbidden-list-guard-always-leaks）。#712 实测了这个盲点的真实代价：在 renderer 壳里共用调用**之
//     后**补 `const M = Math` 加 `M.max(7, …)`，共用调用数仍是 1、`Math.` 计数仍是 0，3 个 suite 67 条
//     全绿，而任何窄于 7px 的 Region 被静默拉宽。此前这里写着「绕过它的写法…要么产出与共用判定不同的
//     取值而被行为层抓住」——那句话在第 2 层存在之前是假的，因为当时没有任何断言驱动那两个壳，本文件的
//     行为层只测纯函数。现在补上第 2 层之后它才成立，且成立的原因在**那两个文件**里，不在这儿。
