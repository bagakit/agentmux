...
describe('Global Agents attention inbox', () => {
  ...
  it('orders Needs you before results and working rows', async () => {
    ...
    expect(groups.map((group) => group.dataset.bucket)).toEqual(['needs-you', 'working', 'done'])
    expect(container.querySelector('[data-session-id="needs-you"]')).toBeTruthy()
  })

  it('clicking a row routes through the existing selectSession owner', async () => {
    ...
    expect(selectSession).toHaveBeenCalledWith('needs-you')
  })
