...
it('keeps Leader Topic on one fixed floating/compact launcher and one canonical Topic', () => {
  ...
  const floatingButtonStart = styles.indexOf('.leader-topic-floating-launcher__button {')
  const floatingButtonEnd = styles.indexOf('.leader-topic-floating-launcher__button:hover')
  ...
  expect(entry).toContain('requestLeaderTopicFloatingClose')
  ...
  expect(styles).toContain('top: -5px; right: -5px')
