...
——**那条路径从不逃出校验函数**。所以判据要钉的是 `cli` 这个绑定**流向谁**（恰好一处：verifyArtifact），
而不是某一种路径拼法的拼写；后者换个写法就绕过去了。这一条若照着原话写进注释，会是一句永远没人
质疑的错话。

**更正（68b67c80）：上面那段说的是判据*应该*钉什么，而不是它当时*实际*钉住了什么。** e2783e92
交付的那一版并没有做到——它匹配的是 `join(root, X)` 这一种拼法加上裸名启动，于是
`` const p = `${root}/${cli.path}` `` 配上 `const launch = spawn` 两行就能绕过去（实测：旧守卫下
「Tests 5 passed」全绿）。也就是说，这段话自己警告的那个形状，它自己就是。68b67c80 用污点传播
（`cli` 起，经 const 扩散，callee 取末段名，跟一级启动器别名）重写之后，钉住的才真是「流向谁」。
判据表里那一行始终成立（文件在、实测 `Test Files 2 passed / Tests 14 passed`）；失真的只是这段叙述。
