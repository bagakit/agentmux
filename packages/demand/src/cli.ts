...
export async function runDemandCli(argv: readonly string[], io: DemandCliIO = {}): Promise<number> {
  ...
  try {
    ...
    stdout(JSON.stringify({ schemaVersion: DEMAND_CLI_SCHEMA, operation: parsed.command, ...result }))
    return 0
  } catch (error) {
    ...
    const output = JSON.stringify({ schemaVersion: DEMAND_CLI_SCHEMA, ok: false, error: serialized })
    stderr(output)
    return 1
  }
}
...
function parseArgs(argv: readonly string[]): ParsedArgs {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (!value || value.startsWith('--')) throw new DemandStoreError('INVALID_INPUT', 'validate', '', `Missing value for --${key}`)
