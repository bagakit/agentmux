...
describe('PMO Teams CLI discovery surface', () => {
  ...
  it('keeps the documented surface connected to the executable dispatcher', async () => {
    ...
    const pmoSource = source.slice(source.indexOf('async function pmoCommand'), source.indexOf('async function demandCommand'))
    expect(pmoSource).toContain('Promise.allSettled')
    expect(pmoSource).toContain('projectsReceipt, agentsReceipt, demandsReceipt] = await Promise.allSettled')
    expect(pmoSource).toContain('unavailable')
  })
})
