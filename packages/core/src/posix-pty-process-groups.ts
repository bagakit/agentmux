...
function parseProcessRows(output: string): ProcessRow[] {
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (pid > 0 && pgid > 1) rows.push({ pid, pgid, tty: match[3]! })
  }
  return rows
}
...
export function getPosixPtyProcessGroups(output: string, rootPid: number, currentPid = process.pid): number[] | null {
  const rows = parseProcessRows(output)
  const root = rows.find((row) => row.pid === rootPid)
  if (!root || root.tty === '?' || root.tty === '??') return null
  // A development daemon can inherit its launch TTY. Never group-signal the
  // daemon itself; fall back to node-pty's already-scoped root kill.
...
export function forceKillPosixPtyProcessGroups(
  ...
  const signalProcessGroup = deps.signalProcessGroup ?? ((pgid: number) => process.kill(-pgid, 'SIGKILL'))
  let firstError: unknown
  for (const pgid of groups) {
    try {
      signalProcessGroup(pgid)
    } catch (error) {
      if (!isProcessAlreadyGone(error) && firstError === undefined) firstError = error
    }
  }
