...
6. 空白 Run 在 handshake 后、initial composer 前保持不可提交且没有 prompt byte 到达 PTY；post-handshake empty composer 可产生一次 initial readiness，无 initial epoch、已消费 epoch、不同 operation 并发全部失败关闭；长期存活的另一 Client 可采纳下一次 native Stop，assistant 行里的 `›`、历史 screen 中与 payload 相同的文本都不能伪造当前 composer，post-boundary live readiness 与 payload/submit crash recovery 都只提交一次；
