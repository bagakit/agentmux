...
| Mounted Desktop | `pnpm --filter @agentmux/desktop package:mac` | 构建 DMG、只读挂载、复制安装并经 LaunchServices 启动真实 packaged Desktop；证明 revision save、外部冲突、原子替换故障和 bundled no-replace move helper。 |
...
| Terminal/Explorer interaction | Fast suite 固定纯黑 Graphite 工作面、完整 ANSI 色槽、OSC parser、历史 Run 只读 fit、平台路径、Workspace Root reveal 边界与 Store action；Production build 证明 Search/WebLinks/WebGL/Context Menu 可打包 |
...
| Workspace atomic move | Main owner tests 在 destination-absent 后、最终 syscall 前制造竞争目标，并覆盖父目录换根、no-follow、no-replace、typed unsupported 和 syscall 后未知终态；Store tests 覆盖 save mutation admission、目标 owner 冲突、错误回执不改投影与一次性路径 rekey；packaged oracle 证明安装副本使用同一 bundled helper。 |
