...
AgentMux 只测试自己的公共 Core/Client 行为和 CtxMux public boundary，不读取 CtxMux wire、数据库、PID table 或私有模块。PTY、Replay、Signal 与 Stop 的底层正确性由固定 CtxMux artifact Gate 拥有；AgentMux 另做 checkout-external composition proof，证明打包和映射没有破坏这些能力。
...
| Mounted Desktop | `pnpm --filter @agentmux/desktop package:mac` | 构建 DMG、只读挂载、复制安装、经 LaunchServices 启动真实 packaged Desktop，并走 Renderer → preload → IPC → `WorkspaceFiles` 的 revision save、外部失效、Reload/Overwrite、删除、read error 与原子替换故障 oracle。 |
...
| Terminal/Explorer interaction | Fast suite 固定黑底 Graphite 的 Agent Surface 层次、Terminal options、OSC parser、历史 Run 只读 fit、平台路径、Workspace Root reveal 边界与 Store action；Production build 证明 Search/WebLinks/WebGL/Context Menu 可打包 |
...
| daemon activation ownership | spawned `ctxmuxd` 的 readiness、public handshake 与 `owner.json` 原子提交处于同一 cleanup guard；receipt rename 失败的 checkout-external fixture 要求精确 child 在 `connect()` 返回前完成 TERM/KILL + reap，且不遗留临时 receipt。Desktop 测试程序另启动一个没有 `owner.json` 的真实 daemon，仅凭测试专属随机 socket owner、artifact path 与完整 argv 精确识别并清理，覆盖 Desktop 在 readiness 与 receipt 之间崩溃的窗口。 |
