...
已读不表示运行时恢复。收件通知保留当前问题、影响和恢复动作；相同原因持续更新而非重复堆积，只有恢复后再发生、原因或严重程度变化、新 Run 的问题才重新提醒。切换 Tab、重挂载和应用重启不能把相同已读问题重新变成未读。已解决条目随权威事实消失，不维护第二份 Runtime 状态或无限错误历史。连接、投递核验、队列阻塞和局部操作失败都归入当前 Session 邮箱；局部重试仍然可用。
