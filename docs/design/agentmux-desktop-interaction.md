...
- Project Rail 的密度由用户在 Projects 标题行就地切换（加号旁），不进设置页；切换只改变行高、行图标尺寸与每层缩进，**不改变树结构、归属、选中项或滚动位置**——密度是看法，不是数据。该选择重启后仍然有效（视觉档位见 DEN「Project Rail 与 Topic 行密度」）。
...
- 头像中央表示 Provider，颜色叠加与左上自定义角标**按执行器配置**；右上表示状态，右下表示数量。菜单与头像共用状态语义，running（在线）与 working（正在处理）可区分。正常完成、主动打断与主动停止不得冒充真实错误；未知原因如实表达。
...
- 自定义角标必须能在执行器设置实际编辑、保存和重新显示，位置与状态角标分开；视觉位置与风格见密度 SSOT 本次复查。
...
- 用户明确：自定义 Icon 直接在 Executor／Execution 配置里，这不算显示配置，而是为了标识 Executor。颜色叠加与左上自定义角标和该 Executor 一起编辑、保存；外观设置不再另放一份入口或配置。
