...
- 展开/收起入口放在快捷工具行最左侧，与文件、截屏、技能、指令等按钮同排；展开态不额外占用一整行高度，收起后仍保留同一位置的一键恢复入口与草稿提示。
- 展开入口必须参与底部工具行的正常布局流，不能以浮动定位覆盖输入文字；收起态仍在该行同一位置提供恢复按钮。
...
- Message Tools 使用独立于队列的图标和三态展开：收起、恢复当前高度、向上展开更多工具；输入中的 skill/component/subcommand 以带下划线的语义 token 呈现，显示短名，hover 显示完整引用，点击打开对应交互或预置 prompt。
