...
用户要求“右下角的所有图标，包括发送和打断也放一起”，现状“不够整体”。用量、发送、打断、邮箱与 Agent 头像共用一个固定右侧操作组；各自点击目标保持明确，打开邮箱不发送也不打断。打断只中断当前回复并保留会话，图标和提示不得与结束整个 Session 混淆；终止 Session 仍归原生命周期入口。忙闲、三种输入框形态和窄分屏不改变操作组高度或右侧落点。视觉语言见密度 SSOT 的同名约束。
