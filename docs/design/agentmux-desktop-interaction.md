...
- **实时输出通道断了、进程没死，是这条原则的第四个消费方**。重连成功了，但这一个 Run 的输出泵没能重建：输入照常送达 Agent，它的回显却永远到不了这块屏。这是第 2 类里最容易被漏掉的一种——**没有任何一步报错**，界面看上去完全正常，用户对着一块永不回显的屏幕继续打字。所以放行的同时必须如实说明"输出可能没在这里显示，但你的输入仍然送达"，并给出恢复动作（Resume 重新 attach）。这条事实是 run 作用域、可持久的，挺过视图切换与快照刷新，直到一次成功的 reattach 撤下它——它不能是一条会被下一张会话快照洗白的瞬时错误，否则用户只在最初那一瞬有机会看见。
