...
用户反馈“Reading the terminal output history didn’t complete 一出现，跟狗皮膏药一样关都关不了”，要求 Message Tools 有类似 Mailbox 的入口。Session 的服务提醒必须可由用户收起到通知收件箱；收起表示已读，不表示运行时恢复。收件箱保留当前问题、影响与恢复动作，未解决时入口仍可辨认，不能把降级静默隐藏。相同问题持续更新而非重复堆积或反复弹出；只有问题已恢复后再次发生、原因或严重程度实际变化，才重新提醒。切换 Tab、重挂载和应用重启不能让已读的同一问题重新霸占画面。已解决条目随权威事实消失，不维护第二份 Runtime 状态或无限错误历史。

收件箱在三种输入框形态都可访问，不因 Agent 忙碌、输入禁用或窄分屏而不可用；空时可打开并明确无当前提醒。它统一收纳当前 Session 的服务提醒、投递队列阻塞与局部操作错误，不在不同位置重复铺设无法关闭的报错。待发送消息及其重试、复制、移除操作继续属于发送队列；通知提供简短原因和去向。局部操作错误保留已有重试能力。控件语言见密度 SSOT。
