...
三个点是头像上的叠加操作按钮，不再作为头像右侧的独立远端按钮。头像徽章不使用绿色描边，改用半透明毛玻璃面和轻阴影表达层级；绿色只保留在龙头像和注意力状态上。浮动态点击必须沿用同一份 open/close 状态，不能因为 pointer capture 或隐藏面板层把点击吞掉。compact 入口打开面板时，必须以底部入口为锚点，把面板夹在靠近底边的可用区域内，不复用随机的历史 `position`。
