...
### Composer 静息密度与主操作
静息态是**一行**：空输入框不占两行高。一行态是最安静的形态，其控件密度也必须最低——切换键在一行态更矮、不带重边框；两行与大输入框档位才允许更强的存在感。Send 与 Interrupt 以图标呈现，但破坏性动作与常规发送之间的视觉区分不得随之消失（颜色语汇仍按 Design Control Language：绿=推进、红=破坏性）。
