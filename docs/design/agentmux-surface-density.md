...
- Project Rail 还要把状态槽和选择槽分开：选中行使用中性 Surface 填充与文字层级，运行中的 Agent 使用独立的小状态点/状态槽；不得用选中态的图标增亮来暗示“只有这个项目在运行”。普通 Project 行去掉重复的文件夹图标，标题从统一的左缘开始；只有有身份区分价值的特殊工作区图标可以保留。`Projects` 分组标签使用 `--fs-micro` 的元信息尺度与轻字重，不得比项目行标题更醒目。目录分组头同样受这条约束（它是个可折叠控件，但字重仍压在项目名之下）——把它做得更响来求"区分"是反的，区分归角色与形态，见下表《Project Rail Group Header》。
...
- 分组成员的缩进是一个轻量层级信号：成员整体向内一格，嵌套成员在此基础上再向内；不增加左侧竖线、边框或高亮，不改变项目行高度，也不让分组头比项目标题更响。未分组项目保持原左缘，保证扫读时能一眼看出分组边界。
