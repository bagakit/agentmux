...
三个点叠在龙头像的右上角，使用独立的 20px 命中区和不透明度变化区分“更多操作”，不再横向拉开入口宽度。头像外层使用 `backdrop-filter` 毛玻璃、半透明 surface 和柔和阴影，不画绿色描边；绿色只作为品牌图形和注意力语义。compact 态打开面板时，面板从底部切换器上方或邻近侧面出现，并在视口边界内夹紧，保持入口与面板的视觉连接。
