...
### Composer 静息密度与主操作
静息态是**一行**：空输入框不占两行高。一行态是最安静的形态，其控件密度也必须最低——切换键在一行态同高但更安静、不带重边框；两行与大输入框档位才允许更强的存在感。Send 与 Interrupt 以图标呈现，但破坏性动作与常规发送之间的视觉区分不得随之消失（颜色语汇仍按 Design Control Language：绿=推进、红=破坏性）。
...
- 密度档由用户拥有，控件落在 Projects 标题行的加号旁；它是**可见的常驻控件**，不是隐藏偏好，也不进设置页——用户要在看着树的同时调。档位数保持极少（默认 + 更紧），不做无级滑块：滑块要求用户自己找一个好值，而这件事只有两三个好值。
...
### Message Tools 通知收件箱（2026-09-20）

通知收件箱使用可辨认的 Inbox 图标，固定在 Message Tools 左侧控件簇；当前提醒数量与未读状态必须可辨，不使用持续跳动或反复弹出。入口与切换键等高，在一行态也不消失。展开使用受视口约束的局部浮层；关闭提醒后不再占输入框上方空间，仍可从收件箱查看。行为约束见交互 SSOT 的“Message Tools 通知收件箱”。
