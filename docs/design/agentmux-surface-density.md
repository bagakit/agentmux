...
### 全局工作面标题栏留白与 Tasks 标签（2026-09-24）

Focus 与 Tasks 在 Project Rail 缺席时，顶栏标题从系统窗口控制区右侧开始；macOS 的三色按钮和文字不能叠放。让位属于顶栏 chrome，使用与已有窗口按钮留白相同的几何来源，不在 Focus、Tasks 各自的内容面板里另加一份边距。非 macOS 不因 macOS 按钮预留无意义的空白，窄窗中次级面包屑可截断，主名称仍可读。

底部导航、面包屑与工作面工具栏统一显示 `Tasks`；Demand 卡片、详情和操作仍可用 Demand 指代具体持久化对象。`Tasks` 表达用户来此推进工作，具体状态列和 Demand 身份不因名称变化而换一套视觉语言。行为约束见交互合同“全局工作面顶栏与 Tasks 命名”。
