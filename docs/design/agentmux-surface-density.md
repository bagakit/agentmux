...
右侧用量、发送、打断、邮箱／头像读作同一条紧凑操作组：共用连续底色与圆角边界，内部按钮静息无独立色块或光晕，悬停和键盘焦点仍明确。头像在该组内以状态色保留运行状态，不再画独立发光边框；待处理／错误标识仍保留。发送是上箭头，打断是举手而非实心方块，避免与结束 Session 相似。点击区沿用 24px 等高约束；各态右侧内边距和组高度一致。行为约束见交互 SSOT 同名小节。
