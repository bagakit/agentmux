...
发件列表原有复制/手动重试仍在异常分支调用全局报错，违反同一邮箱的已批准闭合。沿用本 Feature 的局部错误验收，补齐两个出口到现有 feedback owner，并验证失败不写全局 error、内容不丢、再次成功清除局部提醒。无新的队列状态或通知分发层。

补查验证：邮箱 9 条与局部反馈 17 条通过，两个新增异常出口变异均变红（本轮合计 19 个）；Desktop typecheck 通过。再次执行 clipboard-copy 仍只有上述既有 TerminalView 清单差异。
